"""Train continuous Prompt-Aligned Injection with UNet LoRA on Pendulum."""
...
def _labels_unit(batch: Dict[str, Any], attr_cols: Sequence[str], device: torch.device, dtype: torch.dtype) -> torch.Tensor:
    vals = [batch[k].to(device=device, dtype=dtype).view(-1) for k in attr_cols]
    return torch.stack(vals, dim=1)


class PAIConditioner:
    def __init__(
        self,
        pipe,
        mapper: PromptAlignedInjection,
        prompt: str,
        slot_tokens: Sequence[str],
        device: torch.device,
        dtype: torch.dtype,
    ):
        self.pipe = pipe
        ...
        self.device = device
        self.dtype = dtype
        self.slot_positions = find_slot_positions(pipe.tokenizer, prompt, slot_tokens).to(device)
    ...
    def encoder_hidden_states(self, labels: torch.Tensor) -> torch.Tensor:
        prompt = self.prompt_embeds(labels.shape[0]).to(dtype=self.dtype)
        return self.mapper(prompt, labels, self.slot_positions)

    def causal_tokens(self, labels: torch.Tensor) -> torch.Tensor:
        return self.mapper.causal_tokens(labels.float()).to(device=self.device, dtype=self.dtype)


def save_checkpoint(mapper: PromptAlignedInjection, output_dir: Path, global_step: int, cfg: Dict[str, Any]) -> None:
    ...
    torch.save(payload, output_dir / "pai_mapper.pt")
...
def causal_token_contrastive_loss(
    ...
    # Pendulum values are continuous. Positives are other samples of the same
    # causal variable; unlike binary CelebA, exact value equality is not useful.
    labels = labels.float().reshape(-1)
    if mode == "attr":
        positive = attr_ids[:, None].eq(attr_ids[None, :])
    elif mode == "attr_value":
        tolerance = 0.05
        positive = attr_ids[:, None].eq(attr_ids[None, :]) & ((labels[:, None] - labels[None, :]).abs() <= tolerance)
    else:
...
def save_hybrid_checkpoint(pipe, mapper: PromptAlignedInjection, output_dir: Path, global_step: int, cfg: Dict[str, Any]) -> None:
...
def main() -> None:
    ...
    logger = get_logger("train_pai_lora_pendulum")
    ...
    # transformers>=5 expects this mapping, while the cached MiniSD safety
    # checker was authored against the older `_tied_weights_keys` interface.
    try:
    ...
    attr_cols = list(cfg.get("pendulum_attr_cols", PENDULUM_ATTRS))
    prompt = str(cfg.get("pai_prompt", "an image is @ and * and & and #"))
    slot_tokens = list(cfg.get("pai_slot_tokens", ["@", "*", "&", "#"]))
    if len(slot_tokens) != len(attr_cols):
        raise ValueError("pai_slot_tokens must have one entry per pendulum_attr_cols")
    ...
    optimizer = torch.optim.AdamW(
        [
            {"params": list(mapper.parameters()), "lr": float(cfg.get("pai_lr", cfg.get("lr", 1e-3)))},
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            labels = _labels_unit(batch, attr_cols, device, dtype=torch.float32)
            with torch.no_grad():
