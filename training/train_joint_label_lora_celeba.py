"""Train joint four-label cross-attention tokens with MiniSD UNet LoRA."""
...
from models.prompt_aligned_injection import JointLabelTokenInjection, find_slot_positions
...
class PAIConditioner:
    def __init__(
        self,
        pipe,
        mapper: JointLabelTokenInjection,
        prompt: str,
    ...
    def encoder_hidden_states(self, labels_pm1: torch.Tensor) -> torch.Tensor:
        ...
        return self.mapper(prompt, labels_pm1, self.slot_positions)

    def causal_tokens(self, labels_pm1: torch.Tensor) -> torch.Tensor:
        return self.mapper.causal_tokens(labels_pm1.float()).to(device=self.device, dtype=self.dtype)


def save_checkpoint(mapper: JointLabelTokenInjection, output_dir: Path, global_step: int, cfg: Dict[str, Any]) -> None:
...
def save_hybrid_checkpoint(pipe, mapper: JointLabelTokenInjection, output_dir: Path, global_step: int, cfg: Dict[str, Any]) -> None:
...
def main() -> None:
    ...
    logger = get_logger("train_joint_label_lora_celeba")
    ...
    prompt = str(cfg.get("pai_prompt", "a human is @ and * and & and #"))
    slot_tokens = list(cfg.get("pai_slot_tokens", ["@", "*", "&", "#"]))
    if len(slot_tokens) != len(attr_cols):
        raise ValueError("pai_slot_tokens must have one entry per celeba_attr_cols")
    ...
    logger.info(
        f"Training joint label tokens + LoRA for {max_train_steps} steps; "
    ...
    logger.info(f"Finished joint label tokens + LoRA at step={global_step}; saved {final_dir}")
